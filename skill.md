...
name: rag-to-skill
description: |
  從 JSONL（RAG 源文件）建立 ai Code Skill 的完整工作流程工具。
  ...
  - 使用者有 .jsonl 檔案並想建立可分享的 ai Code skill
...
> **自動增量檢測規範**：在對任何 Skill 進行開發前，**必須**先檢查目標輸出目錄（如 `~/.agents/skills/<skill-name>` 或 `C:\Users\User\.agents\skills\<skill-name>`）是否已存在 `skill.md` 與 `references/` 目錄。若已存在，**務必自動判定為增量更新情境（情境 B）**。此時嚴禁執行初始化或覆寫原有目錄，必須採用增量追加（Append）的方式更新，以保留既有檔案的排版、標題及人工調整內容。
...
2. **目標輸出目錄**：新 skill 的放置路徑（預設建議為 `$HOME/.agents/skills/` 或 Windows 下的 `%USERPROFILE%\.agents\skills\`）
...
腳本位置：`~/.agents/skills/rag-to-skill/`（或同 repo 根目錄）。
