...
1. **明確指出文獻來源**：列出精確的書本章節名稱、段落標題或影片名稱（例如：《<書名>》第 X 章）。
2. **精確的行號 (Line Number) 超連結**：在回答中，必須針對引用的關鍵概念或金句，插入指向本技能 `references/` 目錄中具體參考檔案的超連結，且該超連結必須包含 `#L` 精確行號定位。
   * 範例：`[01-<group1>.md#L123](file:///<skills_root>/<skill_name>/references/01-<group1>.md#L123)`
3. **精確的 RAG 區塊 item_index**：標明原始 JSONL 數據庫對應的 `RAG item_index`（例如：`RAG item_index=11.95`）。
