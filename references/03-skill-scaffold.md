...
SKILL_DIR="${USERPROFILE:-$HOME}/.agents/skills/$SKILL_NAME"
...
| 檔案 | 內容 | 何時讀 |
|---|---|---|
| `references/01-<group1>.md` | <描述涵蓋範圍> | <使用者提問哪類問題時> |
| `references/02-<group2>.md` | <描述涵蓋範圍> | <使用者提問哪類問題時> |
...
當您使用本技能回答使用者提問時，**必須強制且嚴格地標註所有觀點與內容的來源出處**。請遵守以下最高標註規範：
1. **使用易讀的章節與時間/段落格式**：不要在回覆中呈現 RAG item_index（例如 `RAG item_index=11.95`）或直接顯示原始檔案名稱行號（例如 `[01-simple-living.md#L480]`），這對使用者而言難以閱讀與理解。
2. **採用「章節名稱與時間/段落」標示**：
   * 對於書籍：標明 `"chapter": "章節名稱", "section": "段落標題/頁碼"`。
   * 對於影片/音檔：標明 `"chapter": "影片標題", "section": "時間區間 [HH:MM:SS - HH:MM:SS]"`。
3. **將超連結隱藏於標題中（區分線上網址與本地檔案）**：
   * **有線上原始網址（如 Threads 貼文、YouTube 影片、網頁等）**：**必須直接使用原始網址（URL）作為超連結連結目標**（例如 `https://www.threads.net/...` 或 `https://www.youtube.com/watch?v=...`）。請從檢索到的 reference 檔案錨點中提取該網址，不要使用 `file:///` 本地連結。例如：
     * `[章節名稱](https://www.threads.net/...)`
   * **無線上原始網址（如實體書、電子書等）**：指向本地 `references/` 目錄中參考檔案的 `#L` 行號超連結。例如：
     * `[章節名稱](file:///<skills_root>/<skill_name>/references/01-<group1>.md#L123)`

此來源標註規則為本技能的最高回覆鐵律，回答中不可有例外，請務必確實遵守。
```
