...
1. **使用易讀的章節與時間/段落格式**：不要在回覆中呈現 RAG item_index（例如 `RAG item_index=11.95`）或直接顯示原始文件名行號（例如 `[01-simple-living.md#L480]`），這對使用者而言難以閱讀與理解。
2. **採用「章節名稱與時間/段落」標示**：
   * 對於書籍：標明 `"chapter": "章節名稱", "section": "段落標題/頁碼"`。
   * 對於影片/音檔：標明 `"chapter": "影片標題", "section": "時間區間 [HH:MM:SS - HH:MM:SS]"`。
3. **將超連結隱藏於標題中**：將精確指向 `references/` 目錄中參考檔案的 `#L` 行號超連結，直接嵌入到章節名稱或段落名稱中。例如：
